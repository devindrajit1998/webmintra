      ...
      <div className="grid items-start gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="space-y-4 lg:sticky lg:top-24">
          <section className="rounded-lg border border-slate-800 bg-[#0b1826] p-5">
            <div className="flex items-center gap-4 lg:block lg:text-center">
              <div className="group relative mx-0 h-16 w-16 shrink-0 lg:mx-auto lg:h-20 lg:w-20">
                ...
                <label
                  className="absolute inset-0 flex cursor-pointer items-center justify-center rounded-full bg-slate-950/75 text-white opacity-0 transition group-hover:opacity-100 group-focus-within:opacity-100"
          ...
          <section className="rounded-lg border border-slate-800 bg-[#0b1826] p-4">
            ...
            <div className="mt-3 space-y-3">
              ...
              {profile.phone ? (
                <StatusRow label="Phone" verified={Boolean(profile.isPhoneVerified)} />
              ) : null}
        ...
        <div className="space-y-6">
          <form
            ...
            className="overflow-hidden rounded-lg border border-slate-800 bg-[#0b1826]"
          >
            <SectionHeader
          ...
          >
              ...
              <div className="sm:col-span-2">
                ...
                >
                  {[1, 2, 3, 4].map((level) => (
                    <span
                      key={level}
                      className={`h-1 rounded-full ${level <= passwordStrength.score ? passwordStrength.color : "bg-slate-800"}`}
                    />
                  ))}
                </div>
              </div>
            ...
            <FormActions>
              <p className="flex items-center gap-1.5 text-xs text-slate-500">
                <KeyRound className="h-3.5 w-3.5" /> Changing your password requires the current
                password.
              </p>
              <button
                type="submit"
...
function StatusRow({ label, verified }: { label: string; verified: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3 text-xs">
      ...
      <span
        className={`inline-flex items-center gap-1.5 font-medium ${verified ? "text-emerald-300" : "text-amber-300"}`}
      >
...
function SectionHeader({
  ...
  return (
    <div className="flex items-start gap-3 border-b border-slate-800 px-5 py-4 sm:px-6">
      <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-slate-800 text-emerald-300">
        {icon}
      </span>
      <div>
...
function FormActions({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-3 border-t border-slate-800 bg-slate-950/20 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      {children}
    </div>
  );
}
