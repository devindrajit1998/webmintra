    ...
    <form onSubmit={handleSubmit} className="space-y-6 pb-10">
      <header className="flex flex-col gap-5 border-b border-slate-800 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
            Manage the identity and contact details used across your websites and customer
            touchpoints.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-700 px-4 text-xs font-bold text-slate-300 transition hover:border-slate-500 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"
          >
          ...
          <button
            type="submit"
            disabled={!hasChanges || isSaving}
            className="inline-flex h-10 items-center gap-2 rounded-lg bg-emerald-400 px-5 text-xs font-bold text-slate-950 transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-40"
          >
      ...
      <section className="overflow-hidden rounded-xl border border-slate-800 bg-[#0b1826]">
        <div className="flex flex-col gap-6 border-b border-slate-800 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-xs text-slate-500">{business.email || user.email}</p>
            </div>
          </div>
          <div className="w-full max-w-sm">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-slate-300">Profile completeness</span>
              <span className="font-bold text-emerald-300">{completion}%</span>
            </div>
        ...
        <div className="grid divide-y divide-slate-800 lg:grid-cols-[minmax(0,1fr)_360px] lg:divide-x lg:divide-y-0">
          <div className="p-5 sm:p-6">
            <SectionHeading
              title="Brand assets"
              description="Upload the visual identity shown across your web presence."
            />
          ...
          <aside className="bg-slate-950/20 p-5 sm:p-6">
            <SectionHeading
              title="Brand preview"
              description="A quick view of your saved identity."
            />
            <div className="mt-6 overflow-hidden rounded-lg border border-slate-700 bg-white">
              <div className="flex h-10 items-center gap-2 border-b border-slate-200 bg-slate-100 px-3">
                ...
                <div className="ml-2 flex h-6 min-w-0 flex-1 items-center gap-2 rounded bg-white px-2 text-[9px] text-slate-400">
                  {business.faviconUrl ? (
              ...
              <div className="flex h-20 items-center gap-3 px-5">
                {business.logoUrl ? (
                  ...
                  <span className="flex h-9 w-9 items-center justify-center rounded bg-slate-900 text-sm font-black text-white">
            ...
            <p className="mt-4 flex items-start gap-2 text-xs leading-5 text-slate-500">
              ...
              Your favicon is applied to published sites after you save this profile.
            </p>
          </aside>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-slate-800 bg-[#0b1826] p-5 sm:p-6">
        <SectionHeading
          ...
          description="Public-facing information customers use to identify and contact your business."
        />
        <div className="mt-6 grid gap-x-5 gap-y-6 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="space-y-2 md:col-span-2 xl:col-span-3">
            <label htmlFor="business-address" className="text-xs font-semibold text-slate-300">
              Business address
            </label>
            <div className="relative">
              ...
              <textarea
                ...
                className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950/70 py-3 pl-10 pr-4 text-sm text-slate-200 outline-none transition placeholder:text-slate-600 focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
              />
            </div>
            <Counter value={business.address.length} max={300} />
          </div>
          <div className="space-y-2 md:col-span-2 xl:col-span-3">
            <label htmlFor="business-description" className="text-xs font-semibold text-slate-300">
              Business description
            </label>
            <textarea
              ...
              className="w-full resize-y rounded-lg border border-slate-700 bg-slate-950/70 px-4 py-3 text-sm leading-6 text-slate-200 outline-none transition placeholder:text-slate-600 focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
            />
...
function AssetUploader({
  ...
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-950/35 p-4">
      <div>
      ...
      <button
        ...
        className={`mt-4 flex w-full items-center justify-center overflow-hidden rounded-lg border border-dashed border-slate-600 bg-slate-950/50 transition hover:border-emerald-400 hover:bg-emerald-400/5 ${square ? "h-36" : "h-36"}`}
      >
        {isUploading ? (
      ...
      <div className="mt-3 flex gap-2">
        <button
          ...
          onClick={() => inputRef.current?.click()}
          disabled={isUploading}
          className="inline-flex h-8 flex-1 items-center justify-center gap-2 rounded-md border border-slate-700 text-[11px] font-bold text-slate-300 hover:bg-slate-800"
        >
        ...
        {value ? (
          <button
            ...
            onClick={() => onChange("")}
            className="flex h-8 w-8 items-center justify-center rounded-md border border-rose-400/20 text-rose-300 hover:bg-rose-400/10"
          >
      ...
      <label className="mt-3 block">
        ...
        <input
          ...
          className="h-9 w-full rounded-md border border-slate-700 bg-slate-950/70 px-3 text-[11px] text-slate-300 outline-none placeholder:text-slate-600 focus:border-emerald-400"
        />
      </label>
      <input
        ref={inputRef}
...
function BrandMark({ business }: { business: BusinessInfo }) {
  return business.logoUrl ? (
    <span className="flex h-14 w-20 shrink-0 items-center justify-center rounded-lg border border-slate-700 bg-white p-2">
      <img
        src={business.logoUrl}
        alt="Business logo"
    ...
    <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-emerald-400 text-xl font-black text-slate-950">
      {business.name.slice(0, 1).toUpperCase() || "B"}
    </span>
  );
}
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="h-11 w-full rounded-lg border border-slate-700 bg-slate-950/70 pl-10 pr-4 text-sm text-slate-200 outline-none transition placeholder:text-slate-600 focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
        />
      </div>
    </div>
  );
...
function LoadingState() {
  return (
    <div className="grid min-h-[55vh] place-items-center rounded-xl border border-slate-800 bg-[#0b1826]">
      <div className="text-center">
        <Loader2 className="mx-auto h-6 w-6 animate-spin text-emerald-300" />
        <p className="mt-3 text-sm text-slate-500">Loading business profile...</p>
      </div>
    </div>
  );
}
function ErrorState({ message }: { message: string }) {
  return (
    <div className="grid min-h-[55vh] place-items-center rounded-xl border border-rose-400/20 bg-rose-400/5 p-6 text-center">
      <div>
        <h1 className="font-display text-xl font-bold text-rose-300">
        ...
        <button
          ...
          className="mt-5 rounded-lg bg-rose-400 px-4 py-2 text-sm font-bold text-slate-950"
        >
          Try again
        </button>
      </div>
