...
function AnnouncementsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getAnnouncements({ page, limit: 10, isImportant: isImportant ? isImportant === "true" : undefined }),
  });
    ...
    <div className="w-full">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Broadcast messages and updates to all tenants.</p>
        </div>
        <button 
        ...
        <form onSubmit={handleSubmit} className="mb-6 rounded-xl border border-slate-800 bg-[#0b1826] p-5">
          ...
          <div className="grid gap-4">
            <label className="text-xs font-medium text-slate-300">Title
              <input required value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="mt-1 h-10 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 text-sm" placeholder="Announcement Title" />
            </label>
            <label className="text-xs font-medium text-slate-300">Content
              <textarea required value={form.content} onChange={e => setForm({...form, content: e.target.value})} className="mt-1 h-32 w-full rounded-lg border border-slate-700 bg-slate-900 p-3 text-sm" placeholder="Message content..." />
            </label>
            <div className="flex gap-4">
              <label className="flex-1 text-xs font-medium text-slate-300">Type
                <select value={form.type} onChange={e => setForm({...form, type: e.target.value})} className="mt-1 h-10 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 text-sm">
              ...
              <label className="flex items-center gap-2 text-xs font-medium text-slate-300 pt-6">
                <input type="checkbox" checked={form.isImportant} onChange={e => setForm({...form, isImportant: e.target.checked})} className="rounded border-slate-700 bg-slate-900 text-cyan-500 focus:ring-cyan-500" />
      ...
      <div className="rounded-xl border border-slate-800 bg-[#0b1826]">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 p-4">
          <div className="flex w-full items-center gap-2 sm:w-auto">
            <div className="relative">
              <select 
                value={isImportant}
                onChange={e => { setIsImportant(e.target.value); setPage(1); }}
        ...
        <div className="divide-y divide-slate-800/70 p-4">
          {isLoading ? (
            <div className="py-10 text-center text-slate-500"><div className="flex flex-col items-center justify-center gap-3 py-8"><Loader2 className="h-8 w-8 animate-spin text-cyan-500" /><p className="text-sm text-slate-500">Loading announcements...</p></div></div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <div className={`mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
                      ...
                      announcement.type === 'error' ? 'bg-rose-500/10 text-rose-400' :
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {announcement.isImportant && <span className="rounded bg-rose-500/10 px-1.5 py-0.5 text-[9px] font-bold uppercase text-rose-400">Important</span>}
                      </div>
                      <p className="mt-1 text-sm text-slate-400">{announcement.content}</p>
                      <p className="mt-2 text-xs text-slate-500">Broadcasted on {new Date(announcement.createdAt).toLocaleString()} by {announcement.author?.name || 'System'}</p>
                    </div>
                  </div>
