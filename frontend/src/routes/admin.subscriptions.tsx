    ...
    <div className="w-full">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">View and manage tenant subscription lifecycles.</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-800/70">
              {isLoading ? (
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500"><div className="flex flex-col items-center justify-center gap-3"><Loader2 className="h-6 w-6 animate-spin text-cyan-500" /><p className="text-sm text-slate-500">Loading subscriptions...</p></div></td></tr>
              ) : data?.subscriptions?.length ? (
                data.subscriptions.map((sub: SubscriptionRow) => (
                  <tr key={sub.id} className="transition-colors hover:bg-slate-800/20">
                    <td className="px-6 py-4">
                      <p className="font-medium text-slate-200">{sub.tenant?.businessName || sub.tenant?.name || "Unknown Tenant"}</p>
                      <p className="text-xs text-slate-500">{sub.tenant?.email}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="font-medium text-slate-200">{sub.plan?.slug === "pro" ? "Business" : sub.plan?.name || "Unknown Plan"}</p>
                      <p className="text-xs text-slate-500">₹{sub.plan?.price} / {sub.plan?.interval}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium capitalize ${sub.status === 'active' ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400' :
                        sub.status === 'trialing' ? 'border-cyan-500/20 bg-cyan-500/10 text-cyan-400' :
                          sub.status === 'cancelled' ? 'border-rose-500/20 bg-rose-500/10 text-rose-400' :
                ...
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500">No subscriptions found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
