    ...
    <div className="w-full">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Send in-app notifications to specific users or broadcast to all.</p>
        </div>
        <button 
          ...
          <div className="grid gap-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-xs font-medium text-slate-300">Title
                <input required value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="mt-1 h-10 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 text-sm" placeholder="Notification Title" />
              </label>
              <label className="text-xs font-medium text-slate-300">Target User (Optional)
                <input value={form.targetUser} onChange={e => setForm({...form, targetUser: e.target.value})} className="mt-1 h-10 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 text-sm" placeholder="User ID (leave blank to broadcast)" />
              </label>
            </div>
            <label className="text-xs font-medium text-slate-300">Message
              <textarea required value={form.message} onChange={e => setForm({...form, message: e.target.value})} className="mt-1 h-24 w-full rounded-lg border border-slate-700 bg-slate-900 p-3 text-sm" placeholder="Notification content..." />
            </label>
            <label className="text-xs font-medium text-slate-300 w-full sm:w-1/2">Type
      ...
      <div className="rounded-xl border border-slate-800 bg-[#0b1826]">
        <div className="divide-y divide-slate-800/70 p-4">
          {isLoading ? (
            <div className="py-10 text-center text-slate-500"><div className="flex flex-col items-center justify-center gap-3 py-8"><Loader2 className="h-8 w-8 animate-spin text-cyan-500" /><p className="text-sm text-slate-500">Loading notifications...</p></div></div>
              ...
              <div key={notification.id || notification._id} className="py-4 first:pt-0 last:pb-0">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {!notification.targetUser && <span className="rounded bg-slate-700 px-1.5 py-0.5 text-[9px] font-bold uppercase text-slate-300">Broadcast</span>}
                      </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button className="text-slate-500 hover:text-rose-400" title="Delete"><Trash2 className="h-4 w-4" /></button>
                  </div>
                </div>
              </div>
            ))
