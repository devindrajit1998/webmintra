...
import { Loader2, Activity, Search, Filter, Shield, Database, Trash2, Edit3, LogIn, AlertTriangle } from "lucide-react";
...
function ActivityLogsPage() {
  ...
  const getActionIcon = (logAction: string) => {
    ...
    if (logAction.includes('update') || logAction.includes('edit')) return <Edit3 className="h-4 w-4" />;
    if (logAction.includes('login') || logAction.includes('auth')) return <LogIn className="h-4 w-4" />;
    if (logAction.includes('failed') || logAction.includes('error')) return <AlertTriangle className="h-4 w-4" />;
    ...
    <div className="w-full">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Comprehensive audit trail of platform events.</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1000px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-800/70">
              {isLoading ? (
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500"><div className="flex flex-col items-center justify-center gap-3"><Loader2 className="h-6 w-6 animate-spin text-cyan-500" /><p className="text-sm text-slate-500">Loading audit logs...</p></div></td></tr>
              ) : data?.logs?.length ? (
                data.logs.map((log: any) => (
                  <tr key={log.id} className="transition-colors hover:bg-slate-800/20">
                    <td className="px-6 py-4">
                      <div className="flex items-start gap-3">
                        <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md border ${getActionColor(log.status)}`}>
                          {getActionIcon(log.action)}
                        </div>
                        <div>
                          <p className="font-medium text-slate-200">{log.action}</p>
                          <p className="text-[10px] text-slate-500 line-clamp-1">{log.details ? JSON.stringify(log.details) : "No details"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <p className="text-[9px] text-slate-500 line-clamp-1" title={log.userAgent}>{log.userAgent?.split(' ')[0]}</p>
                    </td>
                ...
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500">No activity logs found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
