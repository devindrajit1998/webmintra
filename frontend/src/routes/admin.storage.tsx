...
import { Loader2, Search, Filter, HardDrive, Image as ImageIcon, FileText, Trash2, Database, AlertCircle, Cloud } from "lucide-react";
...
function StoragePage() {
  ...
  const handleDelete = (id: string) => {
    if (confirm("Are you sure you want to delete this file? This action cannot be undone and may break tenant websites if the file is in use.")) {
    ...
    <div className="w-full">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Monitor and manage tenant file uploads and storage limits.</p>
        </div>
      </div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-4">
        <article className="rounded-xl border border-indigo-900/50 bg-[#0b1826] p-4">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-500/15 text-indigo-400"><Cloud className="h-5 w-5" /></span>
            <div>
              ...
              <div className="flex items-center gap-2">
                <p className="font-display text-xl font-bold text-indigo-400">
                  {ikLoading ? <Loader2 className="h-4 w-4 animate-spin text-indigo-400" /> : formatBytes(ikStats?.totalBytes || 0)}
                </p>
              </div>
            </div>
          </div>
        ...
        <article className="rounded-xl border border-slate-800 bg-[#0b1826] p-4">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-cyan-400/15 text-cyan-300"><HardDrive className="h-5 w-5" /></span>
            <div>
        ...
        <article className="rounded-xl border border-slate-800 bg-[#0b1826] p-4">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-400/15 text-emerald-300"><Database className="h-5 w-5" /></span>
            <div>
              <p className="text-[11px] text-slate-500">Total Files</p>
              <p className="font-display text-xl font-bold text-slate-200">{data?.globalStats?.fileCount?.toLocaleString() || 0}</p>
            </div>
          </div>
        </article>
        <article className="rounded-xl border border-rose-900/50 bg-[#0b1826] p-4">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-rose-400/15 text-rose-300"><AlertCircle className="h-5 w-5" /></span>
            <div>
              <p className="text-[11px] text-slate-500">Unused Files</p>
              <p className="font-display text-xl font-bold text-rose-400">{data?.globalStats?.unusedBytes ? formatBytes(data.globalStats.unusedBytes) : 0}</p>
            </div>
          </div>
        </article>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-800/70">
              {isLoading ? (
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500"><div className="flex flex-col items-center justify-center gap-3"><Loader2 className="h-6 w-6 animate-spin text-cyan-500" /><p className="text-sm text-slate-500">Loading files...</p></div></td></tr>
              ) : data?.items?.length ? (
                data.items.map((file: any) => (
                  <tr key={file.id} className="transition-colors hover:bg-slate-800/20">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm text-slate-300">{file.tenant?.businessName || "Unknown"}</p>
                ...
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500">No storage items found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
