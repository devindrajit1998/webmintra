...
export function Btn({
  ...
  const variants: Record<string, string> = {
    ...
    ghost: "text-muted-foreground hover:text-foreground hover:bg-elevated border border-transparent",
...
export function SectionTitle({ children, hint }: { children: ReactNode; hint?: string | undefined }) {
  return (
    <div className="mb-3 flex items-baseline justify-between gap-3">
      <h3 className="font-display text-xs font-bold tracking-[0.16em] text-muted-foreground uppercase">{children}</h3>
...
export function TextInput({
  ...
  return (
    <label className="block">
      {label ? <span className="mb-1.5 block text-[11px] font-semibold text-muted-foreground">{label}</span> : null}
      {multiline ? (
        <textarea rows={4} className={base} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} />
      ) : (
        <input type={type} className={base} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} />
      )}
    </label>
  );
}
...
export function ConfidenceChip({ level }: { level: "High" | "Medium" | "Low" }) {
  return <Chip tone={level === "High" ? "success" : level === "Medium" ? "warning" : "danger"}>{level}</Chip>;
}
...
export function EmptyState({ icon, title, body, action }: { icon: ReactNode; title: string; body: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/70 px-6 py-12 text-center">
      <div className="text-primary mb-3 grid h-11 w-11 place-items-center rounded-xl bg-primary/10">{icon}</div>
