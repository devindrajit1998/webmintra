  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="flex flex-col w-full max-w-2xl max-h-[85vh] rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
        ...
        {/* Assets Grid */}
        <div className="relative flex-1 overflow-y-auto p-4 min-h-[260px]">
          {uploading && (
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-slate-950/80 backdrop-blur-xs animate-in fade-in">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/15 text-primary border border-primary/30 shadow-glow">
                <Loader2 className="h-6 w-6 animate-spin" />
              </div>
              <div className="text-center">
                <p className="text-xs font-semibold text-foreground">Uploading image to library...</p>
                <p className="text-[11px] text-muted-foreground">Optimizing and storing asset</p>
              </div>
            </div>
          )}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <label className="flex items-center justify-center gap-1.5 cursor-pointer rounded-lg border border-border bg-card px-3 py-2 text-xs font-semibold text-foreground transition hover:border-primary/50 shadow-xs">
              {isDirectUploading ? <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" /> : <Upload className="h-3.5 w-3.5 text-muted-foreground" />}
              <span>{isDirectUploading ? "Uploading..." : "Direct Upload"}</span>
              <input
                type="file"
                accept="image/*"
                className="hidden"
